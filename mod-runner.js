...
window.pixelBugMod.receive(async request => {
  try {
    if (!cachedRunner || cachedKind !== request.kind || cachedCode !== request.code) {
      cachedKind = request.kind;
      cachedCode = request.code;
      cachedRunner = request.kind === "brush" ? compileBrush(request.code) : compileEffect(request.code);
    }
    const result = await cachedRunner(request.payload);
    window.pixelBugMod.respond(request.id, result);
  } catch (error) {
    window.pixelBugMod.respond(request.id, null, error?.message || String(error));
  }
});
