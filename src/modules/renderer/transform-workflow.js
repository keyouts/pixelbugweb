(() => {
  function create(options) {
    ...
    function open() {
      ...
      session = { box: { ...selectionBox }, data: clone(data), mask: EditorFeatures.cloneMask(currentSelectionMask(), projectWidth(), projectHeight()), ratio: selectionBox.w / selectionBox.h, rotation: 0 };
    ...
    function layerSelectionData(targetLayer, box, maskSet) {
      ...
      for (let y = 0; y < box.h; y++) for (let x = 0; x < box.w; x++) {
        ...
        if (!EditorFeatures.maskHas(maskSet, sourceX, sourceY)) continue;
        mask[y][x] = true;
    ...
    function apply() {
      ...
      frameIndexes.forEach(frameIndex => {
        ...
        layerIndexes.forEach(layerIndex => {
          ...
          if (!transformOptions.duplicate) {
            EditorFeatures.forEachMaskPoint(session.mask, (x, y) => {
      ...
      const nextMask = EditorFeatures.createMask(projectWidth(), projectHeight());
      for (let y = 0; y < transformed.h; y++) for (let x = 0; x < transformed.w; x++) if (transformed.mask[y]?.[x]) {
        const targetX = transformOptions.x + x;
        const targetY = transformOptions.y + y;
        if (targetX >= 0 && targetY >= 0 && targetX < projectWidth() && targetY < projectHeight()) nextMask.addPoint(targetX, targetY);
      }
