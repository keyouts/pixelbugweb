...
for (const name of ["fetch", "XMLHttpRequest", "WebSocket", "EventSource", "importScripts", "postMessage", "close"]) {
  try { Object.defineProperty(self, name, { value: undefined, configurable: false, writable: false }); } catch (_error) {}
}
...
async function handleMessage(event) {
  ...
  try {
    const runner = request.kind === "brush" ? compileBrush(String(request.code || "")) : compileEffect(String(request.code || ""));
    const result = await runner(request.payload);
    sendMessage({ id: request.id, result });
  } catch (error) {
    sendMessage({ id: request.id, error: error?.message || String(error) });
  } finally {
